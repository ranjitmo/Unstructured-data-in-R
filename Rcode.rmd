...
# Note that base R 'data.frame' is used instead of tidyverse 'data_frame' as that resolves an issue with geocoding we would otherwise encounter
...
# we could write the resulting data frame to our working directory
# path <- getwd()
# write.csv( crash_df, paste0(path, "/crash_data.csv") ) 
...
With the example code above we scraped only two out of the 2,500 or so pages across which all the airline accident data is spread if we really wanted to gather all of it going back to 1918. We are happy with roughly the past 20 years worth of data - 1997 to 2017 - and I scraped that in a separate session, geocoded location and wrote the resulting data frame into "crash_data.csv". We can now just read that back in and are ready to proceed.

So let's visualize the locations of airline crashes via bubbles on a worldmap and let size of the bubbles represent the number of fatalities. We will use the package leaflet that provides zooming, panning and popups out of the box.
...
m <- leaflet(crash_df) %>%
  ...
  addCircles(lng = ~lon, lat = ~lat, stroke = FALSE, fillColor = "red", fillOpacity = 0.3,
    radius = ~Fatalities * 1000, popup = paste(                                               # we add some inline styling to format the 
      paste0("<h3 style=","color:black;", ">", crash_df$Location, "</h3>"),                   # apperance of the popups
